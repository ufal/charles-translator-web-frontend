...
const hacekCarkaTransformer = (
  ...
  if (pluginState.nextWithHacek && hacekMapping[lWhichKey] !== undefined) {
    ...
    whichKey = state.capsLockEnabled
      ? hacekMapping[lWhichKey].toUpperCase()
      : hacekMapping[lWhichKey];
  }
  ...
  if (pluginState.nextWithCarka && carkaMapping[lWhichKey] !== undefined) {
    ...
    whichKey = state.capsLockEnabled
      ? carkaMapping[lWhichKey].toUpperCase()
      : carkaMapping[lWhichKey];
  }
...
const settingsKey = {
  ...
  fn: (pluginState, setPluginState) => {
    const isOpening = (pluginState) => {
      return (
        pluginState.settingsShown === true ||
        pluginState.settingsShown === undefined
      );
    };
...
export const keyboardConfig = {
        ...
        letterLines: [
          ...
          [
            ...
            "L",
            {
              keyName: "ˇ",
              fn: (pluginState, setPluginState) =>
                hacekCarkaFeature(pluginState, setPluginState, "ˇ"),
            },
          ],
          [
            "SHIFT",
            ...
            "M",
            {
              keyName: "´",
              fn: (pluginState, setPluginState) =>
                hacekCarkaFeature(pluginState, setPluginState, "´"),
            },
            "BACKSPACE",
          ],
          lastLine,
  ...
  curLayoutIndex: 0,
  // shown: false, // UI detail, none of our business here
  transformers: [
    (state, pluginState, setPluginState, whichKey) => {
      return hacekCarkaTransformer(
        state,
        pluginState,
        setPluginState,
        whichKey,
  ...
  // TODO: get the model files and figure out their distribution
  // (maybe link to github releases?)
