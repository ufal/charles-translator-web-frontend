...
export default function ASR(props) {
    ...
    React.useEffect(() => {
        setState({ ...state, visible: navigator !== undefined })
    }, [])
    ...
    React.useEffect(() => {
        ...
        if (ASR && ASR.current) ASR.current.stop()

        ASR.current.onresult = (result) => {
            props.onresult('')
        }
        initASR()
    }, [props.language])
    ...
    const initASR = () => {
        ...
        speechRecognition.onresult = function (result) {
            ...
            if (props.onresult !== undefined) props.onresult(transcript)

            if (result.final && props.onfinal !== undefined) props.onfinal(transcript)
        }
        ...
        speechRecognition.onerror = function (e) {
            ...
            if (props.onerror !== undefined) props.onerror(e)
        }
            ...
            {state.visible && (
                <Tooltip className={styles.removeButton} title={state.active ? t("asr:stopRecording") : t("asr:translateByVoice")}>
                    <IconButton
                        ...
                        onClick={() => {
                            setState((oldState) => {
                                if (ASR.current === null) initASR()

                                if (oldState.active) ASR.current.stop()
                                else {
